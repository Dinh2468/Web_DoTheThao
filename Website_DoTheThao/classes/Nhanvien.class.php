...
class Nhanvien
{
    ...
    // Các thuộc tính chi tiết của nhân viên (tương tự như Khachhang)
    public $nhanvien_id;
    public $taikhoan_id;
    public $ho_ten;
    public $email;
     ...
     * @return array Danh sách các đối tượng nhân viên.
     */
    public function readAll()
    {
        ...
        // Trả về trực tiếp mảng các đối tượng từ DB::select()
    ...
    public function create($data)
    {
        ...
        try {
            // ... (Phần INSERT taikhoan giữ nguyên) ...

            $taikhoan_id = $this->db->lastInsertId();

            // 2. Thêm vào bảng nhanvien (CẬP NHẬT CỘT)
    ...
    public function update($id, $data)
    {
        ...
        try {
            ...
            // 1. Cập nhật bảng nhanvien (CẬP NHẬT CỘT)
            $ho_ten = $this->db->escapeString($data['ho_ten']);
            $email = $this->db->escapeString($data['email']);
            $sdt = $this->db->escapeString($data['sdt']);
            $chuc_vu = $this->db->escapeString($data['chuc_vu']);
            ...
            // ... (Phần cập nhật mật khẩu giữ nguyên) ...
    ...
    /**
     * Cập nhật trạng thái tài khoản nhân viên (Khóa/Mở khóa).
     */
    public function updateStatus($id, $status)
    {
        ...
        // 1. Lấy taikhoan_id từ nhanvien_id
        $taikhoan_info = $this->db->selectOne("SELECT taikhoan_id FROM " . $this->table_nhanvien . " WHERE nhanvien_id = $safe_id");
        ...
        return $this->db->execute($sql); // Dùng DB::execute()
    }
    ...
    // === CÁC HÀM KIỂM TRA TÍNH DUY NHẤT (UNIQUE) ===
