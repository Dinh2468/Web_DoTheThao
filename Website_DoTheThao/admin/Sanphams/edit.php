...
// Nhúng Controller cho Loại SP và NCC (để lấy danh sách cho dropdown)
...
$current_data = null; // Dữ liệu sản phẩm hiện tại
$loai_sanphams = $loaispController->index()['loai_sanphams']; // Lấy danh sách Loại SP
$nha_cung_caps = $nccController->index()['nhacungcaps']; // Lấy danh sách NCC
...
if ($sanpham_id > 0) {
    ...
    // Nếu không có ID, chuyển hướng
    $_SESSION['error_message'] = "ID Sản phẩm không hợp lệ.";
    header('Location: index.php');
    exit();
}

// Khởi tạo các biến form (dùng cho trường hợp xử lý lỗi)
$form_data = [
    ...
    'hinh_anh_hien_tai' => $current_data->hinh_anh, // Tên ảnh hiện tại
];
...
// 2. XỬ LÝ FORM SUBMISSION (Cập nhật)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Lấy dữ liệu từ form POST
    $post_data = $_POST;

    // Gộp dữ liệu POST và dữ liệu ảnh hiện tại
    ...
    if ($update_result['success']) {
        ...
        // Cập nhật thất bại, giữ lại dữ liệu và hiển thị lỗi
        $errors = $update_result['errors'] ?? [];
        $message = $update_result['message'];
        $message_type = 'error';

        // Gán lại dữ liệu POST vào form_data để người dùng không phải nhập lại
